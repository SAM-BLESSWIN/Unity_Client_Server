...
namespace GameServer
{
    class Server
    {
        ...
        public static void Start(int maxplayers, int port)
        {
            ...
            Console.WriteLine("Listening for client " + DateTime.Now.ToString());
            Console.WriteLine("Server started on " + Port);
        }

        public static void TCPconnectioncallback(IAsyncResult result)
        {
            Console.WriteLine("connected to a client "+DateTime.Now.ToString());
           // Console.WriteLine("TCPconnectioncallback: "+result.ToString());
            ...
            Console.WriteLine("incoming connection from socket" + client.Client.RemoteEndPoint);
            ...
            for (int i = 1; i <= MAX_player; i++) //checks the socket is null if null connect with client
            {
                if (clients[i].tcp.Socket==null)  //checks the socket in tcp class in client class in client script
                {
                    ...
                    //Console.WriteLine("clientid: "+i+" "+"client value:"+clients[i]);
                    return;
                }
            }
        ...
        public static void UDPreceivecallback(IAsyncResult _result)
        {
            try
            {
                ...
                using(Packet packet=new Packet(data))
                {
                    ...
                    //Console.WriteLine($"UDP-clientid:{clientid}");
            ...
            {
                Console.WriteLine($"Error in UDP receivecallback: {ex}");
            }
        }
        ...
        public static void SendUDPData(IPEndPoint _clientendpoint,Packet _packet)
        {
            ...
            {
                Console.WriteLine($"Error UDP-Sending data : {ex}");
            }
        }

        private static void Initializeserverdata()
        {
            Console.WriteLine("Server Data Initializing...");
